...
#test if the current node is headnode
if test_is_headnode == 1:
    ...
    try:
        check_output(['/usr/bin/anaconda/bin/conda', 'update', 'setuptools'])
    except sp.CalledProcessError as e:
        print(e.output)
        raise e

    command = ['/usr/bin/anaconda/bin/conda', 'install']
    ...
    command = ['/usr/bin/anaconda/bin/pip', 'install']
    ...
    call(['/usr/bin/anaconda/bin/jupyter', 'serverextension', 'enable', '--user', '--py', 'jupyter_spark'])
    call(['/usr/bin/anaconda/bin/jupyter', 'nbextension', 'install', '--user', '--py', 'jupyter_spark'])
    call(['/usr/bin/anaconda/bin/jupyter', 'nbextension', 'enable', '--user', '--py', 'jupyter_spark'])
    call(['/usr/bin/anaconda/bin/jupyter', 'nbextension', 'enable', '--user', '--py', 'widgetsnbextension'])
    ...
    with open('/lib/systemd/system/jupyter.service', 'w') as f:
        opts = [
            ...
            'ExecStart=/usr/bin/anaconda/bin/python /usr/bin/anaconda/bin/jupyter notebook --allow-root',
            'Restart=always',
            'RestartSec=1',
            '[Install]',
